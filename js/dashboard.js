...
const Dashboard = {
    ...
    updateChildInfo() {
        if (this.childInfo) {
            ...
            document.getElementById('child-gender').textContent = this.childInfo.gender === 'male' ? 'Мальчик' : 'Девочка';
    ...
    updateRecentMeasurements() {
        ...
        if (measurements && measurements.length > 0) {
            ...
            measurementsContainer.innerHTML = `
                <p>Пожалуйста, заполните данные о физическом развитии ребенка в разделе <a href="#growth">Физическое развитие</a>.</p>
            `;
        }
    },
            ...
            <form id="edit-child-info-form">
                ...
                <div class="form-group">
                    ...
                    <select id="edit-child-gender" required>
                        <option value="male" ${this.childInfo.gender === 'male' ? 'selected' : ''}>Мальчик</option>
                        <option value="female" ${this.childInfo.gender === 'female' ? 'selected' : ''}>Девочка</option>
                    </select>
                </div>
